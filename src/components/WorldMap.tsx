...
export const WorldMap: React.FC<WorldMapProps> = ({ data, activities, onCountryClick, onActivityClick, selectedId }) => {
  ...
  // Independent ref to track the last transform state specifically for Globe logic
  // to avoid conflicts with the shared transformRef which might be used for rendering/state
  const globeLastTransformRef = useRef<d3.ZoomTransform>(d3.zoomIdentity);
  ...
  // Intro Animation Sequence with Onboarding Tooltips
  const [onboardingStep, setOnboardingStep] = useState<'step1' | 'step2' | 'done'>('step1');

  useEffect(() => {
    // Wait for map data to load before starting the intro
    if (!data) return;

    console.log('[Onboarding] Starting intro animation sequence...');

    // Start with Step 1 (Globe -> Flat)
    const t1 = setTimeout(() => {
      console.log('[Onboarding] Step 1: Switching to Flat view');
      setViewState(ViewState.FLAT);
      setOnboardingStep('step2'); // Show "Switch back to Globe" hint
    }, 1500);

    // Wait in Flat mode (3s), then toggle back
    const t2 = setTimeout(() => {
      console.log('[Onboarding] Step 2: Switching back to Globe view');
      setViewState(ViewState.GLOBE);
    }, 1500 + 3000); // 4.5s total

    // Brief delay before hiding the last tooltip
    const t3 = setTimeout(() => {
      console.log('[Onboarding] Step 3: Hiding tooltips');
      setOnboardingStep('done');
    }, 1500 + 3000 + 1500); // 6s total
    ...
    data.features.forEach(feature => {
      try {
        ...
        if (centroid && !isNaN(centroid[0]) && !isNaN(centroid[1])) {
          centroids.set(feature.properties.name, centroid);
        }
      } catch (e) {
        console.warn(`Could not calculate centroid for ${feature.properties.name}:`, e);
      }
    });
    return centroids;
  }, [data]);
  ...
  useLayoutEffect(() => {
    ...
    globeLastTransformRef.current = d3.zoomIdentity; // Reset globe tracker
  ...
  // ------------------------------------------------------------------
  // 4. Interaction Logic (Zoom / Drag) - Delegated to Hook
  // ------------------------------------------------------------------
  useMapInteraction({
    svgRef,
    viewState,
  ...
  // ------------------------------------------------------------------
  // 6. Animation Loop (Auto-Rotation) - Delegated to Hook
  // ------------------------------------------------------------------
  useAutoRotation({
    projection,
    viewState,
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center">
        <div className="flex p-1 bg-brandWhite/10 backdrop-blur-xl rounded-full border border-brandWhite/20 shadow-2xl relative">
          <button
          ...
          {/* Onboarding Tooltip: Step 1 - Pointing to Map */}
          {onboardingStep === 'step1' && (
            <div className="absolute top-full right-0 mt-3 flex flex-col items-end animate-in fade-in slide-in-from-top-2 duration-700">
              <div className="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[8px] border-b-brandDeep mr-6"></div>
              <div className="bg-brandDeep border border-brandWhite/20 text-brandCream text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-xl whitespace-nowrap">
                Try Flat View
              </div>
            </div>
          )}

          {/* Onboarding Tooltip: Step 2 - Pointing to Globe */}
          {onboardingStep === 'step2' && (
            <div className="absolute top-full left-0 mt-3 flex flex-col items-start animate-in fade-in slide-in-from-top-2 duration-700">
              <div className="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[8px] border-b-brandDeep ml-6"></div>
              <div className="bg-brandDeep border border-brandWhite/20 text-brandCream text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-xl whitespace-nowrap">
                Back to 3D
              </div>
            </div>
      ...
      <div className="absolute top-36 md:top-20 left-4 z-30 font-sans">
      ...
      {/* Map Legend (Below Filter Button) */}
      <div className="absolute top-48 md:top-32 left-4 z-20 pointer-events-none">
        <div className="bg-brandWhite/10 backdrop-blur-xl border border-brandWhite/20 rounded-xl px-3 py-2 shadow-2xl">
          <div className="text-[9px] font-bold uppercase tracking-widest text-brandGrey mb-2">EVENT</div>
          <div className="flex flex-col space-y-2">
            {/* Upcoming/Today */}
            <div className="flex items-center space-x-2">
              <div className="relative flex items-center justify-center">
                <div className="absolute w-full h-full bg-brandRed/50 rounded-full animate-ping"></div>
                <div className="relative w-3 h-3 rounded-full bg-brandRed border-2 border-white shadow-sm z-10"></div>
              </div>
              <span className="text-[10px] text-brandCream font-medium">Upcoming / Today</span>
            </div>
            {/* Past Events */}
            <div className="flex items-center space-x-2">
              <div className="relative flex items-center justify-center">
                <div className="absolute w-full h-full bg-brandGrey/50 rounded-full animate-ping"></div>
                <div className="relative w-3 h-3 rounded-full bg-white border-2 border-brandRed shadow-sm z-10"></div>
              </div>
              <span className="text-[10px] text-brandCream font-medium">Past Event</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <g transform={groupTransform}>
          ...
          {data.features.map((feature) => {
            ...
            try {
              ...
              console.warn(`Path generation failed for ${feature.properties.name}:`, e);
              return null;
            }
          ...
          {filteredActivities.map((activity) => {
            ...
            // Temporal logic: Determine if event is past, today, or future
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            let isPastEvent = true; // Default to past if no date
            ...
            // Style: Past = White fill/Red stroke, Today/Future = Red fill/White stroke
            const markerFillClass = isPastEvent ? 'fill-white' : 'fill-brandRed';
            const markerStrokeClass = isPastEvent ? 'stroke-brandRed' : 'stroke-white';
            ...
            return (
              ...
              >
                ...
                {!isSelected && (
                  <circle r={8 / scaleFactor} className={`${isPastEvent ? 'fill-brandGrey' : 'fill-brandRed'} opacity-40 animate-ripple pointer-events-none`} />
                )}

                {/* Main Dot - Scaled with Temporal Styling */}
                <circle
                  r={(isSelected || isHovered ? 8 : 5) / scaleFactor}
                  strokeWidth={(isSelected ? 2.5 : 1.5) / scaleFactor}
                  className={`transition-all duration-300 ${isSelected ? 'fill-brandCream stroke-brandRed' : `${markerFillClass} ${markerStrokeClass}`}`}
                />
              </g>
            );
          })}
